...
import { Sparkles, AlertCircle, RefreshCw, ChevronDown } from "lucide-react";
...
export const ScrollVideoHero: React.FC<ScrollVideoHeroProps> = ({
  videoSrc,
  scrollMultiplier = siteConfig.heroScrollMultiplier || 4,
  onProgressChange,
}) => {
  ...
  // Scroll and scrubbing state stored in refs for 60fps performance without React re-render churn
  ...
  // Synchronize duration and ready state
  ...
  // Compute scroll progress from container position
  const updateScrollProgress = useCallback(() => {
    ...
    // Scrolled amount: 0 when container top is at viewport top, 1 when scrolled through
    const scrolledAmount = -rect.top;
    const rawProgress = scrolledAmount / totalScrollableDistance;
    ...
    const renderLoop = () => {
      ...
      if (video && duration > 0 && isLoaded) {
        ...
        if (Math.abs(progressDiff) > 0.0002) {
          currentProgressRef.current += progressDiff * 0.2;
        } else {
          currentProgressRef.current = targetProgressRef.current;
        }

        // Duration-independent time calculation
        const desiredTargetTime = currentProgressRef.current * duration;
        const clampedTargetTime = Math.max(0, Math.min(duration, desiredTargetTime));

        // Unstick seeking lock if it got stalled (> 120ms)
        const now = performance.now();
        if (isSeekingRef.current && (now - lastSeekTimeRef.current > 120 || !video.seeking)) {
          isSeekingRef.current = false;
        }

        // Perform seek if not currently seeking and delta is noticeable
        if (!isSeekingRef.current && !video.seeking) {
          const delta = Math.abs(video.currentTime - clampedTargetTime);
          if (delta > 0.03) {
            try {
        ...
        // Throttle progress state updates to React so UI elements react smoothly
        const roundedProgress = Math.round(currentProgressRef.current * 100) / 100;
        if (Math.abs(roundedProgress - lastReportedProgress) >= 0.01) {
  ...
  // Video source initialization & verification
  useEffect(() => {
    setIsLoaded(false);
    setHasError(false);
    const video = videoRef.current;
  ...
  }, [activeVideoSrc, syncVideoMetadata]);

  const handleVideoError = () => {
    setHasError(true);
    setErrorMessage("امکان پخش آنلاین ویدیوی سینمایی میسر نشد. لطفاً ارتباط اینترنت را بررسی فرمایید.");
  };
  ...
  // Dynamic container height based on scroll multiplier (e.g., 4 = 400vh)
  const containerHeight = `${Math.max(2.5, scrollMultiplier) * 100}vh`;
  ...
  return (
    ...
    >
      {/* Sticky Viewport container (supports dynamic mobile dvh/vh) */}
      <div
        id="hero-sticky-viewport"
        className="sticky top-0 h-screen h-[100dvh] w-full overflow-hidden flex items-center justify-center bg-[#0B1E24]"
      >
        ...
        <video
          ...
          className="absolute inset-0 w-full h-full object-cover object-center filter contrast-[1.05] brightness-[0.96] transition-opacity duration-700 ease-out"
          style={{
        ...
        <div
          ...
          className={`pointer-events-none absolute inset-0 bg-radial from-transparent transition-opacity duration-500 ${
            isDark
              ? "via-black/30 to-black/75 opacity-85"
              : "via-black/20 to-black/55 opacity-65"
          } mix-blend-multiply`}
        />

        {/* Hero Branding & Dynamic Text Overlay */}
        <div
        ...
        >
          ...
          <div className="flex items-center justify-between w-full max-w-7xl mx-auto">
            <div
              className={`flex items-center gap-3 backdrop-blur-md px-4 py-2 rounded-full pointer-events-auto shadow-lg transition-colors ${
                isDark
                  ? "bg-[#0B1E24]/70 border border-[#124A57]/70 shadow-black/30"
                  : "bg-white/90 border border-[rgba(18,74,87,0.2)] shadow-[rgba(18,74,87,0.12)]"
              }`}
            >
              <span className="w-2.5 h-2.5 rounded-full bg-[#CD78B3] animate-pulse" />
              <div className="text-right">
                <span
                  className={`font-extrabold text-sm sm:text-base tracking-wide block ${
            ...
            >
              ...
              <span className="font-light text-[11px] tracking-wide">روایت تعاملی گوشت و استیک ممتاز</span>
            </div>
          </div>

          {/* Center Brand Statement - Fades smoothly on initial scroll */}
          <div
          ...
          >
            <span className="inline-block text-[#CD78B3] text-xs sm:text-sm font-medium tracking-widest px-3.5 py-1 rounded-full bg-[#124A57]/60 border border-[#CD78B3]/40 mb-3 backdrop-blur-md shadow-sm">
          ...
          {/* Bottom Area: Scroll indicator & Timeline progress */}
          <div className="w-full max-w-7xl mx-auto flex flex-col items-center gap-4">
            {/* Scroll Indicator Prompt - Fades out as user scrolls */}
            <div
        ...
        {hasError && (
          ...
          >
            ...
            <h3 className="text-lg font-bold text-white mb-2">امکان پخش ویدیو وجود ندارد</h3>
            <p className="text-sm text-slate-300 max-w-md mb-6">{errorMessage}</p>
            <button
              onClick={handleRetry}
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#124A57] hover:bg-[#1a5b6a] text-white text-sm font-medium transition-colors shadow-lg cursor-pointer"
            >
